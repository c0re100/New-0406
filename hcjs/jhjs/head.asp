...
href="anqi.asp"><font color="#6666FF">暗器</font></a></div></td></tr> </table>
      <table width="100%" border="0" align="center" cellpadding="2" cellspacing="2" bgcolor="#99CCFF">
        <tr align="center" bgcolor="#FF0000"> 
          ...
          <td width="63"> 
            <div align="center"><font color="#FFFFFF">武 器 圖</font></div></td>
          <td width="51"><font color="#FFFFFF">類型 </font> 
          <td width="210"><font color="#FFFFFF">功能</font></td>
          <td width="116"><font color="#FFFFFF">售價</font></td>
...
rs.open "SELECT 物品名,id,類型,等級,防御,堅固度,說明,銀兩 FROM 物品買 where 類型='頭盔' order by 銀兩 ",conn
...
%>
        <tr bgcolor="#FFFFFF"  onmouseout="this.bgColor='#FFFFFF';"onmouseover="this.bgColor='#DFEFFF';"> 
          <form method=POST action='binqi1.asp?id=<%=rs("id")%>'>
            ...
            <td width="63"> <div align="center"><img src="001/<%=rs("說明")%>.gif" border="0" alt="<%=rs("物品名")%> "></div></td>
            <td width="51"> <div align="center"><%=rs("類型")%></div></td>
            <td width="210"> <div align="center">等級需求<%=abs(rs("等級"))%>，加防御<%=abs(rs("防御"))%>，堅固度<%=abs(rs("堅固度"))%></div></td>
            <td width="116"> <div align="center"><%=rs("銀兩")%>兩</div></td>
