...
href="anqi.asp"><font color="#6666FF">暗器</font></a></div></td></tr> </table>
      <table width="100%" border="0" align="center" cellpadding="2" cellspacing="2" bgcolor="#99CCFF">
        <tr align="center" bgcolor="#FF0000"> 
          <td width="62"><font color="#FFFFFF">裝備名</font></td>
          <td width="85"><font color="#FFFFFF"> 武 器 圖</font></td>
          <td width="53"><font color="#FFFFFF">類型 </font> 
          <td width="240"><font color="#FFFFFF">功能</font></td>
          <td width="91"><font color="#FFFFFF">售價</font></td>
...
rs.open "SELECT 物品名,id,說明,等級,類型,防御,攻擊,銀兩  FROM 物品買 where 類型='雙腳' order by 銀兩 ",conn
...
%>
        <tr bgcolor="#FFFFFF"  onmouseout="this.bgColor='#FFFFFF';"onmouseover="this.bgColor='#DFEFFF';"> 
          <form method=POST action='binqi1.asp?id=<%=rs("id")%>'>
            <td width="62"> <div align="center"><%=rs("物品名")%></div></td>
            <td width="85"> <div align="center"><img src="001/<%=rs("說明")%>.gif" border="0" alt="<%=rs("物品名")%> "></div></td>
            <td width="53"> <div align="center"><%=rs("類型")%> </div></td>
            <td width="240"> <div align="center">等級需求<%=abs(rs("等級"))%>，加防御<%=abs(rs("防御"))%>，加攻擊<%=abs(rs("攻擊"))%></div></td>
            <td width="91"> <div align="right"><%=rs("銀兩")%>兩</div></td>
