...
href="anqi.asp"><font color="#6666FF">暗器</font></a></div></td></tr> </table>
      <table border="0" align="center" width="100%" cellpadding="2" cellspacing="2">
        <tr align="center" bgcolor="#CC0000"> 
          <td width="85"><font color="#FFFFFF">裝 備 名</font></td>
          <td width="85"><font color="#FFFFFF"> 武 器 圖</font></td>
          <td width="44"><font color="#FFFFFF">類 型 </font> 
          <td width="263"><font color="#FFFFFF">功 能</font></td>
          <td width="58"><font color="#FFFFFF">售 價</font></td>
          <td colspan="2"><font color="#FFFFFF">數 量</font></td>
          <td width="46"><font color="#FFFFFF">操 作</font></td>
        </tr>
        <%
Set conn=Server.CreateObject("ADODB.CONNECTION")
Set rs=Server.CreateObject("ADODB.RecordSet")
conn.open Application("ljjh_usermdb")
rs.open "SELECT 物品名,id,說明,等級,類型,攻擊,堅固度,銀兩 FROM 物品買 where 類型='右手' order by 等級 ",conn
do while not rs.bof and not rs.eof
%>
        <tr bgcolor="#FFFFFF"  onmouseout="this.bgColor='#FFFFFF';"onmouseover="this.bgColor='#DFEFFF';"> 
          <td width="85"> <div align="center"><%=rs("物品名")%></div></td>
          <form method=POST action='binqi1.asp?id=<%=rs("id")%>'>
            <td width="85"> <div align="center"><img src="001/<%=rs("說明")%>.gif" border="0" alt="<%=rs("物品名")%> "></div></td>
            <td width="44"> <div align="center"><%=rs("類型")%></div></td>
            <td width="263"> <div align="center">等級需求<%=abs(rs("等級"))%>，加攻擊<%=abs(rs("攻擊"))%>，堅固度<%=abs(rs("堅固度"))%></div></td>
            <td width="58"> <div align="center"><%=rs("銀兩")%>兩</div></td>
