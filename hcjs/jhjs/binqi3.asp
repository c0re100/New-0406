...
rs.open "SELECT 類型,物品名,銀兩,類型,內力,體力,攻擊,防御,堅固度,類型,sm,等級 FROM 物品買 where ID=" & ID,conn
if rs("類型")<>"暗器" then
	rs.close
	set rs=noghint
	conn.close
...
If Rs.Bof OR Rs.Eof then
	conn.execute "insert into 物品 (物品名,擁有者,類型,攻擊,防御,堅固度,數量,銀兩,等級,說明,sm,會員) values ('"&wu&"',"& info(9)&",'"& lx&"',"& gj &","& fy &","& jgd &","&sl&","&int(yin*2)&","&dj&",'無',"&sm&","&aaao&")"
else
	id=rs("id")
end if
rs.close
...
response.end
	...
	Response.Write "<script Language=Javascript>alert('提示：購買"&wu&sl&"個成功,如果您是2級以上會員可以打5折！');location.href = 'javascript:history.go(-1)';</script>"
response.end
end if
%>
