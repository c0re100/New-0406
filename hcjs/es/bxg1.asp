...
if zy="" then
	Response.Write "<script Language=Javascript>alert('矗ボ巨ぃΘ叫糶弧');location.href = 'javascript:history.go(-1)';</script>"
	response.end
end if
...
rs.close
rs.open "select id from ユカ初 where 珇='" & wpname & "' and 局Τ="&info(9) & " and よΑ='玂繧耫'",conn
If Rs.Bof OR Rs.Eof then
	conn.execute "insert into ユカ初 (珇,局Τ,よΑ,癸钩,摸,ず,砰,ю阑,ň眘,计秖,弧,も基,丁,蝗ㄢ,sm,绊㏕,单,say,穦) values ('"&wpname&"',"&info(9)&",'玂繧耫','產','"&lx& "',"& nl &","& tl &","& gj &","& fy &","& sl &",'"& say &"',0,now(),"& yin &","& sm &","& jgd &","& dj &",'"& zy &"',"&aaao&")"
else
	id=rs("id")
	conn.execute "update ユカ初 set 计秖=计秖+" & sl & ",穦="&aaao&" where id="&id
end if
rs.close
set rs=nothing
conn.close
set conn=nothing
Response.Write "<script Language=Javascript>alert('矗ボ"& myname &"р禥珇:"& wpname & sl &"玂繧耫');location.href = 'wupin.asp';</script>"
%>
