...
esmoney=rs("二手價")
toname=rs("擁有者")
if duixiang<>info(0) then
	rs.close
	set rs=nothing
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('提示：呵。。笑話，想作什麼呀，這個也不是賣給你的！');location.href = 'javascript:history.go(-1)';</script>"
	response.end
end if
RS.CLOSE
...
conn.close
set conn=nothing
Response.Write "<script Language=Javascript>alert('提示："& myname &"在商貿交易市場購買["& wpname &"]成功！');location.href = 'smjy.asp';</script>"
response.end
%>
