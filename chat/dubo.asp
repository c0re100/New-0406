...
conn.execute "update 用戶 set 銀兩=銀兩+200000 where id="&id
hunyin="倒霉：["& info(0) &"]賭博裡輸給了{"& peiou &"}，10萬兩白花花的銀子飛了！"
end if
if db-Application("ljjh_bingwen")=-1 or db-Application("ljjh_bingwen")=2 then
Response.Write "<script language=JavaScript>{alert('你勝了，哈哈，10萬兩哦~~~');}</script>"
conn.execute "update 用戶 set 銀兩=銀兩+100000 where id="&info(9)
hunyin="恭喜：["& info(0) &"]賭博裡贏了{"& peiou &"}10萬兩銀子！"
end if
if db=Application("ljjh_bingwen") then
Response.Write "<script language=JavaScript>{alert('平了，重新來吧！');}</script>"
conn.execute "update 用戶 set 銀兩=銀兩+100000 where id="&id
