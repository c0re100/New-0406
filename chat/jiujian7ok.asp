...
gwfy=rs("防御")
gwtl=rs("體力")
killer=int((lxjgj1+lxjwg1+egj1+7035000)-gwfy*2)
...
conn.execute "update 用戶 set 銀兩=銀兩-7035000 where id="&info(9)
rs.close
rs.open "select * from 怪物區 where 怪物='"&to1&"'",conn
if rs("kill")<0 then
jingye=int(rs("等級"))
...
stunt=info(0) & "<bgsound src=wav/jiu7.wav loop=1><img src='img/41.gif'>運足內力對"&aa&""&gwclick&"使用了江湖上久已失蹤了5000年的獨孤九劍之破箭式，殺傷"&gwclick&"生命值：" & killer & "點,"&gwclick&"終於不支倒地，慘死在"&meclick&"手上！"&meclick&"獲得"&jingye&"點經驗值！"
call jiujian(stunt)
else
stunt=info(0) & "<bgsound src=wav/jiu7.wav loop=1><img src='img/41.gif'>運足內力對"&aa&""&gwclick&"使用了江湖上久已失蹤了5000年的獨孤九劍之破箭式，殺傷"&gwclick&"生命值：" & killer & "點,"&gwclick&"奮起反擊咬傷"&meclick&"生命值:<font color=red>"&killergw&"</font>點!"
...
conn.close
set conn=nothing
Response.Write "<script Language=Javascript>alert('恭喜，您的破箭式已經發招完成！');location.href = 'javascript:history.go(-1)';</script>"
Response.End
end if
rs.close
...
killer=int((lxjwg1+lxjgj1+egj1+(denji1/r)*35000)-(lxjwg2+lxjfy2+efy2))
if killer<=100 then
randomize timer
killer=int(rnd*99)+1
end if
...
Application.UnLock
conn.execute "insert into 人命(死者,時間,兇手,死因) values ('" & to1 & "',getdate(),'" & info(0) & "','獨孤九劍之破箭式')"
...
stunt=info(0) & "<bgsound src=wav/jiu7.wav loop=1>運足內力對<img src='img/06.gif'><font color=6FBFEF>" & to1 & "</font>使用了江湖上久已失蹤了5000年的獨孤九劍之破箭式，殺傷" & killer & e
call jiujian(stunt)
rs.close
set rs=nothing	
conn.close
...
sd(197)="6FBFEF"
sd(198)="對"
sd(199)="<font color=6FBFEF>【獨孤九劍之破箭式】"& stunt &"</font>"
