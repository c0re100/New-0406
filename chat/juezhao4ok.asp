...
xx2=rs("姓名2")
rs.close
rs.open "select 會員等級,狀態,武功,攻擊,體力,防御,法力,門派,殺人數,保護,操作時間,等級,a1,a2,a3,a4,a5,a6,b1,b2,b3,b4,b5,b6 FROM 用戶 WHERE id="&info(9),conn
jhhy=rs("會員等級")
lxjwg1=rs("武功")
lxjgj1=rs("攻擊")
lxjfy1=rs("防御")
...
if lxjfl1<=3000 then
	...
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('你沒有3000點法力，不能發出這招特技，還是多練練吧！');location.href = 'javascript:history.go(-1)';</script>"
	Response.End
end if
...
killer=int((lxjgj1+lxjwg1+egj1+3000*info(3))-gwfy*2)
...
stunt=info(0) & "<bgsound src=wav/jiuzhao4.wav loop=1><img src='img/1212qqq.gif'>運足內力對"&aa&""&gwclick&"使用了江湖上的曠世絕學<font color=6FBFEF>排雲掌法</font>，隻見風雲色變，殺傷"&gwclick&"生命值：" & killer & "點,"&gwclick&"終於不支倒地，慘死在"&meclick&"手上！"&meclick&"獲得"&jingye&"點經驗值！"
call jiujian(stunt)
else
stunt=info(0) & "<bgsound src=wav/jiuzhao4.wav loop=1><img src='img/1212qqq.gif'>運足內力對"&aa&""&gwclick&"使用了江湖上的曠世絕學<font color=6FBFEF>排雲掌法</font>，隻見風雲色變，殺傷"&gwclick&"生命值：" & killer & "點,"&gwclick&"奮起反擊咬傷"&meclick&"生命值:<font color=red>"&killergw&"</font>點!"
...
conn.close
set conn=nothing
Response.Write "<script Language=Javascript>alert('恭喜，您的排雲掌法已經發招完成！');location.href = 'javascript:history.go(-1)';</script>"
Response.End
end if
rs.close
rs.open "select id,門派,會員等級,武功,殺人數,攻擊,防御,保護,狀態,體力,grade,等級,b1,b2,b3,b4,b5,b6 FROM 用戶 WHERE 姓名='" & to1 & "'",conn
menpai=rs("門派")
jhhy=rs("會員等級")
lxjwg2=rs("武功")
lxjgj2=rs("攻擊")
...
killer=int((lxjwg1+lxjgj1+egj1+(denji1/r)*200)-(lxjwg2+lxjfy2+efy2))
if killer<=100 then
randomize timer
killer=int(rnd*99)+1
end if
...
Application.UnLock
conn.execute "insert into 人命(死者,時間,兇手,死因) values ('" & to1 & "',getdate(),'" & info(0) & "','排雲掌法')"
e="點，" & to1 & "慢慢的倒了下去  從此江湖上又少了一隻大俠,[" & to1 & "]一身功力盡廢，從江湖中消失了,[" & info(0) & "]戰鬥經驗增加"&r&"點," & to1 & "損失戰鬥經驗"&r&"點!.....掉落一些物品，大家快搶!<a href='qiangb.asp?id=" & id&"' target='d'><img src='../hcjs/jhjs/001/"&sm&".gif' border='0' width='32' height='32'  align='top' alt='"&wupin&"'></a>" 
call jiechucolor(idd,id)
else
 e="點"&e1&"" & to1 & "慢慢的倒了下去  從此江湖上又少了一隻大蝦,[" & info(0) & "]戰鬥經驗增加400點!"
end if
else
rs.close
rs.open "select id,物品名,說明 from 物品 where  數量>0 and 裝備=false and 類型<>'卡片' and 擁有者="&idd,conn
if rs.eof or rs.bof then
           e="點"&e1&"" & to1 & "慢慢的倒了下去  從此江湖上又少了一隻大俠,[" & to1 & "]一身功力盡廢，從江湖中消失了.....[" & info(0) & "]戰鬥經驗增加"&r&"點," & to1 & "損失戰鬥經驗"&r&"點!"
else
id=rs("id")
	wupin=rs("物品名")
	sm=rs("說明")
...
Application.UnLock
conn.execute "insert into 人命(死者,時間,兇手,死因) values ('" & to1 & "',getdate(),'" & info(0) & "','排雲掌法')"
e="點，" & to1 & "慢慢的倒了下去  從此江湖上又少了一隻大俠,"&e1&"[" & to1 & "]一身功力盡廢，從江湖中消失了,[" & info(0) & "]戰鬥經驗增加"&r&"點," & to1 & "損失戰鬥經驗"&r&"點!.....掉落一些物品，大家快搶!<a href='qiangb.asp?id=" & id&"' target='d'><img src='../hcjs/jhjs/001/"&sm&".gif' border='0' width='32' height='32'  align='top' alt='"&wupin&"'></a>" 
end if
end if
if info(4)=0 then 
call boot(to1)
...
stunt=info(0) & "運足內力對<img src='img/1212qqq.gif' width='144' height='44'><font color=6FBFEF>" & to1 & "</font><bgsound src=wav/jiuzhao4.wav loop=1>使用了江湖上的曠世絕學<font color=6FBFEF>排雲掌法</font>，隻見風雲色變，" & to1 & "身負重傷,被殺傷" & killer & e
call jiujian(stunt)
rs.close
set rs=nothing	
conn.close
...
sd(197)="6FBFEF"
sd(198)="對"
sd(199)="<font color=6FBFEF>【排雲掌法】"& stunt &"</font>"
