...
if Application("jingda")=1 then
	Response.Write "<script Language=Javascript>alert('目前由於人少的原因繫統被設置為禁打狀態，要想打架請找官府人員開啟！');location.href = 'javascript:history.go(-1)';</script>"
	Response.End
end if
stunt=""
stunt1=""
...
xx2=rs("姓名2")
rs.close
rs.open "select 門派,狀態,殺人數,操作時間,等級,體力,銀兩,武功,攻擊,防御,保護,a1,a2,a3,a4,a5,a6,b1,b2,b3,b4,b5,b6 FROM 用戶 WHERE id="&info(9),conn
...
if yinin<=nei then
	...
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('你沒有"&nei&"兩銀子，不能發出這招特技，還是多賺點錢吧！');location.href = 'javascript:history.go(-1)';</script>"
	Response.End
end if
...
gwfy=rs("防御")
gwtl=rs("體力")
killer=int((lxjgj1+egj1+lxjwg1+1205000)-gwfy*2)
...
gwclick="<font color=blue><a href=javascript:parent.sw('["+gw+"級怪物]'); target=f2>"&aa&""& to1 & "</a></font>"
...
conn.execute "update 用戶 set 銀兩=銀兩-1205000 where id="&info(9)
rs.close
rs.open "select * from 怪物區 where 怪物='"&to1&"'",conn
if rs("kill")<0 then
...
stunt=info(0) & "<bgsound src=wav/jiu1.wav loop=1><img src='img/41.gif'>運足內力對"&aa&""&gwclick&"使用了江湖上久已失蹤了5000年的獨孤九劍之總決式，殺傷"&gwclick&"生命值：" & killer & "點,"&gwclick&"終於不支倒地，慘死在"&meclick&"手上！"&meclick&"獲得"&jingye&"點經驗值！"
call jiujian(stunt)
else
stunt=info(0) & "<bgsound src=wav/jiu1.wav loop=1><img src='img/41.gif'>運足內力對"&aa&""&gwclick&"使用了江湖上久已失蹤了5000年的獨孤九劍之總決式，殺傷"&gwclick&"生命值：" & killer & "點,"&gwclick&"奮起反擊咬傷"&meclick&"生命值:<font color=red>"&killergw&"</font>點!"
...
sd(197)="660099"
sd(198)="對"
sd(199)="<b><font color=red>【繫統】</font></b><font color=blue>["&info(0)&"]</font>大俠武學造詣未成，慘死在"&gw&"級怪物手上!" 
sd(200)=0
...
conn.close
set conn=nothing
Response.Write "<script Language=Javascript>alert('恭喜，您的總決式已經發招完成！');location.href = 'javascript:history.go(-1)';</script>"
Response.End
end if

rs.close
rs.open "select id,門派,武功,攻擊,防御,保護,殺人數,會員等級,狀態,體力,grade,等級,b1,b2,b3,b4,b5,b6 FROM 用戶 WHERE 姓名='" & to1 & "'",conn
menpai=rs("門派")
jhhy=rs("會員等級")
lxjwg2=rs("武功")
lxjgj2=rs("攻擊")
...
if rs("grade")>6 then
	...
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('不可以對官府人員、便衣聊管操作');location.href = 'javascript:history.go(-1)';</script>"
	Response.End
end if
if rs("門派")="出家" then
	rs.close
...
if rs("等級")<=Application("ljjh_xinshou") then
	rs.close
	set rs=nothing
	conn.close
	set conn=nothing
...
killer=int((lxjwg1+lxjgj1+egj1+(denji1/r)*5000)-(lxjwg2+lxjfy2+efy2))
if killer<=100 then
randomize timer
killer=int(rnd*99)+1
end if
...
conn.execute "update 用戶 set 狀態='死',操作時間=now(),lastkick='"&info(0)&"',,allvalue=allvalue-"&r&" where 姓名='" & to1 & "'"
if xx1=info(0) or xx2=info(0) then
...
Application.UnLock
conn.execute "insert into 人命(死者,時間,兇手,死因) values ('" & to1 & "',getdate(),'" & info(0) & "','獨孤九劍之總決式')"
e="點，" & to1 & "慢慢的倒了下去  從此江湖上又少了一隻大蝦,[" & info(0) & "]戰鬥經驗增加"&r&"點," & to1 & "損失戰鬥經驗"&r&"點!掉落一些物品，大家快搶!<a href='qiangb.asp?id=" & id&"' target='d'><img src='../hcjs/jhjs/001/"&sm&".gif' border='0' width='32' height='32'  align='top' alt='"&wupin&"'></a>" 
...
rs.close
rs.open "select id,物品名,說明 from 物品 where ( 類型<>'卡片') and  數量>0 and 裝備=false and 擁有者="&idd,conn
...
Application.UnLock
conn.execute "insert into 人命(死者,時間,兇手,死因) values ('" & to1 & "',getdate(),'" & info(0) & "','獨孤九劍之總決式')"
e="點，" & to1 & "慢慢的倒了下去  從此江湖上又少了一隻大蝦,[" & info(0) & "]戰鬥經驗增加"&r&"點," & to1 & "損失戰鬥經驗"&r&"點!"&e1&"掉落一些物品，大家快搶!<a href='qiangb.asp?id=" & id&"' target='d'><img src='../hcjs/jhjs/001/"&sm&".gif' border='0' width='32' height='32' align='top' alt='"&wupin&"'></a>" 
end if
end if
...
stunt=info(0) & "<bgsound src=wav/jiu1.wav loop=1>運足內力對<img src='img/41.gif'><font color=blue>" & to1 & "</font>使用了江湖上久已失蹤了5000年的獨孤九劍之總決式，殺傷" & killer & e
call jiujian(stunt)
rs.close
set rs=nothing	
conn.close
...
conn.close
set conn=nothing
Response.Write "<script Language=Javascript>alert('錯誤：您正在使用的靈劍江湖不是正版，請支持正版江湖，靈劍總站：http://www.wjyang.com');location.href = 'javascript:history.go(-1)';</script>"
end if
sub jiujian(stunt)
Application.Lock
sd=Application("ljjh_sd")
...
sd(197)="660099"
sd(198)="對"
sd(199)="<font color=red>【獨孤九劍之總決式】"& stunt &"</font>"
