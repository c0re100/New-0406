...
xx2=rs("姓名2")
rs.close
rs.open "select 門派,狀態,殺人數,操作時間,等級,體力,武功,攻擊,防御,保護,銀兩,a1,a2,a3,a4,a5,a6,b1,b2,b3,b4,b5,b6 FROM 用戶 WHERE id="&info(9),conn
...
gwfy=rs("防御")
gwtl=rs("體力")
killer=int((lxjgj1+lxjwg1+egj1+2510000)-gwfy*2)
...
conn.execute "update 用戶 set 銀兩=銀兩-2510000 where id="&info(9)
rs.close
rs.open "select * from 怪物區 where 怪物='"&to1&"'",conn
if rs("kill")<0 then
jingye=int(rs("等級"))
...
stunt=info(0) & "<bgsound src=wav/jiu2.wav loop=1><img src='img/41.gif'>運足內力對"&aa&""&gwclick&"使用了江湖上久已失蹤了5000年的獨孤九劍之破刀式，殺傷"&gwclick&"生命值：" & killer & "點,"&gwclick&"終於不支倒地，慘死在"&meclick&"手上！"&meclick&"獲得"&jingye&"點經驗值！"
call jiujian(stunt)
else
stunt=info(0) & "<bgsound src=wav/jiu2.wav loop=1><img src='img/41.gif'>運足內力對"&aa&""&gwclick&"使用了江湖上久已失蹤了5000年的獨孤九劍之破刀式，殺傷"&gwclick&"生命值：" & killer & "點"&gwclick&"奮起反擊咬傷"&meclick&"生命值:<font color=red>"&killergw&"</font>點!"
...
conn.close
set conn=nothing
Response.Write "<script Language=Javascript>alert('恭喜，您的破刀式已經發招完成！');location.href = 'javascript:history.go(-1)';</script>"
Response.End
end if
rs.close
rs.open "select id,門派,會員等級,狀態,體力,殺人數,grade,等級,保護,武功,攻擊,防御,b1,b2,b3,b4,b5,b6 FROM 用戶 WHERE 姓名='" & to1 & "'",conn
menpai=rs("門派")
jhhy=rs("會員等級")
lxjwg2=rs("武功")
lxjgj2=rs("攻擊")
...
if  rs("門派")="出家" then
	...
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('[" & to1 & "]正在練功或者他已經出家，不能操作！');location.href = 'javascript:history.go(-1)';</script>"
	Response.End
end if
if rs("等級")<=10 then
	rs.close
...
killer=int((lxjwg1+lxjgj1+egj1+(denji1/r)*10000)-(lxjwg2+lxjfy2+efy2))
if killer<=100 then
randomize timer
killer=int(rnd*99)+1
end if
...
Application.UnLock
conn.execute "insert into 人命(死者,時間,兇手,死因) values ('" & to1 & "',getdate(),'" & info(0) & "','獨孤九劍之破刀式')"
...
stunt=info(0) & "<bgsound src=wav/jiu2.wav loop=1>運足內力對<img src='img/06.gif'><font color=blue>" & to1 & "</font>使用了江湖上久已失蹤了5000年的獨孤九劍之破刀式，殺傷" & killer & e
call jiujian(stunt)
rs.close
set rs=nothing	
conn.close
...
sd(197)="6FBFEF"
sd(198)="對"
sd(199)="<font color=6FBFEF>【獨孤九劍之破刀式】"& stunt &"</font>"
