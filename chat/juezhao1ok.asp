...
xx2=rs("姓名2")
rs.close
rs.open "select 門派,狀態,武功,攻擊,防御,體力,銀兩,殺人數,保護,操作時間,等級,a1,a2,a3,a4,a5,a6,b1,b2,b3,b4,b5,b6 FROM 用戶 WHERE id="&info(9),conn
...
gwfy=rs("防御")
gwtl=rs("體力")
killer=int((lxjgj1+lxjwg1+egj1+10000000)-gwfy*2)
...
conn.execute "update 用戶 set 銀兩=銀兩-10000000 where id="&info(9)
rs.close
rs.open "select * from 怪物區 where 怪物='"&to1&"'",conn
if rs("kill")<0 then
jingye=int(rs("等級"))
...
stunt=info(0) & "<bgsound src=wav/dw.wav loop=1><img src='img/41.gif'>運足內力對"&aa&""&gwclick&"使用了江湖上久已失蹤了50000年的分筋錯骨手，殺傷"&gwclick&"生命值：" & killer & "點,"&gwclick&"終於不支倒地，慘死在"&meclick&"手上！"&meclick&"獲得"&jingye&"點經驗值！"
call jiujian(stunt)
else
stunt=info(0) & "<bgsound src=wav/dw.wav loop=1><img src='img/41.gif'>運足內力對"&aa&""&gwclick&"使用了江湖上久已失蹤了50000年的分筋錯骨手，殺傷"&gwclick&"生命值：" & killer & "點,"&gwclick&"奮起反擊咬傷"&meclick&"生命值:<font color=red>"&killergw&"</font>點!"
...
conn.close
set conn=nothing
Response.Write "<script Language=Javascript>alert('恭喜，您的分筋錯骨手已經發招完成！');location.href = 'javascript:history.go(-1)';</script>"
Response.End
end if
rs.close
rs.open "select id,門派,會員等級,武功,殺人數,攻擊,保護,防御,狀態,體力,grade,等級,b1,b2,b3,b4,b5,b6 FROM 用戶 WHERE 姓名='" & to1 & "'",conn
menpai=rs("門派")
jhhy=rs("會員等級")
lxjwg2=rs("武功")
lxjgj2=rs("攻擊")
...
conn.execute "update 用戶 set 武功=500,狀態='死',操作時間=now(),lastkick='"&info(0)&"',allvalue=allvalue-"&r&" where 姓名='" & to1 & "'"
if xx1=info(0) or xx2=info(0) then
...
Application.UnLock
conn.execute "insert into 人命(死者,時間,兇手,死因) values ('" & to1 & "',getdate(),'" & info(0) & "','分筋錯骨手')"
...
Application.UnLock
conn.execute "insert into 人命(死者,時間,兇手,死因) values ('" & to1 & "',getdate(),'" & info(0) & "','分筋錯骨手')"
e="點，" & to1 & "慢慢的倒了下去  從此江湖上又少了一隻大蝦,[" & info(0) & "]戰鬥經驗增加"&r&"點," & to1 & "損失戰鬥經驗"&r&"點!"&e1&"被廢除全部武功...掉落一些物品，大家快搶!<a href='qiangb.asp?id="&id&"' target='d'><img src='../hcjs/jhjs/001/"&sm&".gif' border='0' width='32' height='32'  align='top' alt='"&wupin&"'></a>" 
end if
end if
if info(4)=0 then 
call boot(to1)
...
stunt="<font color=6FBFEF>"&info(0) & "</font>運足內力對<img src='img/43.gif'><font color=6FBFEF>" & to1 & "</font><bgsound src=wav/dw.wav loop=1>使用了江湖上久已失蹤了50000年的<font color=6FBFEF>分筋錯骨手</font>，殺傷" & killer&e
call jiujian(stunt)
rs.close
set rs=nothing	
conn.close
...
sd(197)="6FBFEF"
sd(198)="對"
sd(199)="<font color=6FBFEF>【分筋錯骨手】"& stunt &"</font>"
