...
case else
	...
	if yinliang<100000 or yinliang>2000000 then
		Response.Write "<script language=JavaScript>{alert('最少押：10萬兩，最多200萬！');location.href = 'javascript:history.go(-1)';}</script>"
		Response.End 
	end if
'開始判斷
	...
	if rs("銀兩")<yinliang then
		...
		conn.close
		set conn=nothing
		Response.Write "<script language=JavaScript>{alert('你有這麼多的錢嗎？！');location.href = 'javascript:history.go(-1)';}</script>"
		Response.End 		
	end if
	rs.close
...
set tmprs=nothing
	...
	if rs.eof or rs.bof then
		...
		conn.close
		set conn=nothing
		Response.Write "<script language=JavaScript>{alert('現在沒有莊家，不能進行在線賭博！！');location.href = 'javascript:history.go(-1)';}</script>"
		Response.End 		
	end if
	rs.close
	rs.open "select top 1 * FROM 在線賭博 WHERE 姓名='"& info(0) &"'",conn
	if not(rs.eof) or not(rs.bof) then
		if rs("身份")="莊家" then
			temp=info(0)&"你現在是莊家呀，你要搞什麼呀!"
		else
			temp=info(0)&"你壓["&rs("押大小")&"]"&rs("銀兩")&"等著開吧！"
		end if
		rs.close
		set rs=nothing	
		conn.close
	...
	rs.close
	conn.execute "update 用戶 set 銀兩=銀兩-"&yinliang&" where id="&info(9)
	conn.execute "insert into 在線賭博(姓名,身份,銀兩,押大小,時間) values ('"&info(0) &"','玩家',"&yinliang&",'"&tt&"',now())"	
...
if durs>=10 then
	...
	sjdubo=m1+m2+m3
'查找莊家
rs.open "select 姓名 FROM 在線賭博 WHERE 身份='莊家'",conn
zhuangjia=rs("姓名")
rs.close

'豹子處理
if m1=m2 and m2=m3 and m3=m1 then
	rs.open "select 銀兩 FROM 在線賭博 WHERE 銀兩<>0 and 身份<>'莊家'",conn
	yingyin=0
	...
	xiazhu="莊家開：<img src='../jhimg/"&m1&".gif'><img src='../jhimg/"&m2&".gif'><img src='../jhimg/"&m3&".gif'>計：<font color=red>"& sjdubo &"</font>點!莊家開出豹子  通殺！莊家：<font color=red>"&zhuangjia&"</font>贏得：<font color=red>"&yingyin&"</font>兩！"
		Application.Lock
...
set rs=nothing
		...
		set conn=nothing
		call say(xiazhu)
Response.Write "<script Language=Javascript>alert('下注"&yinliang&"兩成功！');location.href = 'javascript:history.go(-1)';</script>"
Response.End
end if
...
if sjdubo/2=int(sjdubo/2) then
	...
	rs.open "select 銀兩,姓名 FROM 在線賭博 WHERE 押大小='雙'",conn
	do while not rs.bof and not rs.eof
	...
	rs.open "select 銀兩,姓名 FROM 在線賭博 WHERE 押大小='單'",conn
	do while not rs.bof and not rs.eof
...
if sjdubo<=10 then
	...
	rs.open "select 銀兩,姓名 FROM 在線賭博 WHERE 押大小='小'",conn
	do while not rs.bof and not rs.eof
	...
	rs.open "select 銀兩,姓名 FROM 在線賭博 WHERE 押大小='大'",conn
	do while not rs.bof and not rs.eof
...
'daname=""
	...
	xiazhu="莊家開：<img src='../jhimg/"&m1&".gif'><img src='../jhimg/"&m2&".gif'><img src='../jhimg/"&m3&".gif'>計：<font color=red>"& sjdubo &"</font>點!"&danshuang&daxiao&"總下注：<font color=red>"&zong&"</font>兩，莊家：<font color=red>["&zhuangjia&"]</font>收入：<font color=red>"&yingyin &"</font>兩,賠出：<font color=red>"&pei&"</font>兩，合計：<font color=red>"&yingyin-pei&"</font>兩,共有：<font color=red>"&duboname&"</font>玩家幸運押中！"
Application.Lock
	Application("ljjh_dubozhang")=""
Application.UnLock
	set rs=nothing
...
xiazhu="<a href=javascript:parent.sw('[" & info(0) & "]'); target=f2><font color=red>"&info(0)&"</font></a>從自己的腰包裡拿出一張支票,得意地說:這是:<font color=red>"& yinliang &"</font>兩的靈劍江湖銀行支票<img src='pic/Dz01.gif'>，我買"& duboimg &"！，一定中的！現在下注如下：押大：<font color=red>"& dars &"</font>個，押小:<font color=red>"& xiaors &"</font>個,押單：<font color=red>"&danrs&"</font>個,押雙:<font color=red>"&shuangrs&"</font>個！還差:<font color=red>"&(10-durs)&"</font>個開局！"
...
sd(194)=info(0)
sd(195)=info(0)
sd(199)="<font color=green>【銀兩賭局】</font><font color=#FFC01F>"& xiazhu &"</font>"
