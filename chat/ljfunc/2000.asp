...
conn.execute "update 用戶 set 保護=false where 姓名='"&toname&"'"
if info(5)<>"官府" then
aa= info(0) &"[便衣聊管]:<font color=red><bgsound src=wav/daipu.wav loop=1>由於" & toname & "在江湖中違反規則,因此被迫出關!</font>"
else
aa= info(0) &"[官府人員]:<font color=red><bgsound src=wav/daipu.wav loop=1>由於" & toname & "在江湖中違反規則,因此被迫出關!</font>"
end if 
rs.close
	set rs=nothing	
	conn.close
