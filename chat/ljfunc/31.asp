...
if moneyok>2000000000 then
	Response.Write "<script language=JavaScript>{alert('你的存款就快超過了20億了，為防止丟錢現像請您把錢存些進股市吧！');}</script>"
	rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
...
if fn1<=0 then
	if bankmoney=<0 then
		putyin=info(0) & "你在江湖錢莊並沒有存款,錢莊每天利息為:0.0001%,歡迎存取!"
	else
		putyin=info(0) & "在錢莊存有:"& bankmoney &"兩,在:"& rs("結算日期") &"存入,按0.0001%利,銀行現在有:"& newbankmoney &"兩!"
	end if
	rs.close
	set rs=nothing	
	conn.close
...
putyin=info(0) & "向錢莊存錢:"& fn1 &"兩,錢莊現有存款:"& newbankmoney+fn1 &"兩,還要繼續努力!"
