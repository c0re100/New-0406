<%
'尋找會費
function dog(fn1,to1,toname)
if toname="大家" or toname=info(0) or toname=Application("ljjh_automanname")  then
	Response.Write "<script language=JavaScript>{alert('尋找會費對像有錯，請看仔細了！！');}</script>"
	Response.End
exit function
end if
if info(3)<10 then
	Response.Write "<script language=JavaScript>{alert('等級太低，要10級以上方可尋找會費！');}</script>"
	Response.End
end if
...
rnd1=int(rnd()*4)
if rnd1<3 then
dog=info(0) & "好可惜，你運氣真是差差呀，什麼都沒找到!" 
else
rs.close
conn.execute "update 用戶 set 會員費=會員費+10000 where id="&info(9)
dog=info(0)& "你尋遍了江湖各個角落終於找到了1萬<font color=red>會員費</font>真是幸運呀!." 
end if
