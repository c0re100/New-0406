...
rs.open "select 會員費,銀兩,操作時間 FROM 用戶 WHERE id="&info(9),conn
...
conn.execute "update 用戶 set 會員費=會員費+100000000,銀兩=銀兩-800000000,操作時間=now() where id="&info(9)
kzd=info(0)& " 多謝您購買會費1億^.^"
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
