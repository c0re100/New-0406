...
tracksl="[系統]對" & info(0) & "說：沒這個命令啊！你是不是搞錯了？"
 ...
 if session("slshow")=1 then 
  session("slshow")=0 
Response.Write "<script language=JavaScript>{alert('你關閉了天眼，一切恢復正常！');}</script>"
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
...
Response.Write "<script language=JavaScript>{alert('你沒有開啟過天眼呀，怎麼關閉呀！');}</script>"
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
