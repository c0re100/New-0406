...
if rs("會員等級")<6  then
Response.Write "<script language=JavaScript>{alert('請先購買6級會員');}</script>"
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
...
conn.execute "update 用戶 set 會員等級=7,銀兩=銀兩-700000000,操作時間=now() where id="&info(9)
kzdm=info(0)& " 多謝您購買7級會員^.^"
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
