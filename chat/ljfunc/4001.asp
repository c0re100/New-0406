<%
'尋找流星淚
function cccccc(fn1,to1,toname)
if toname="大家" or toname=info(0) or toname=Application("ljjh_automanname")  then
	Response.Write "<script language=JavaScript>{alert('尋水流星淚對像有錯，請看仔細了！！');}</script>"
	Response.End
exit function
end if
if info(3)<10 then
...
rnd1=int(rnd()*4)
if rnd1<3 then
cccccc=info(0) & "好可惜，你尋遍了江湖各個角落也沒有找到什麼流星淚,"&info(0) & "損耗100000點法力!" 
else
rs.close
rs.open "select id FROM 物品 WHERE 物品名='流星淚' and 擁有者=" & info(9)
	If Rs.Bof OR Rs.Eof then
		conn.execute "insert into 物品(物品名,擁有者,類型,堅固度,攻擊,防御,內力,體力,數量,銀兩,說明) values ('流星淚',"&info(9)&",'物品',100,0,0,0,0,1,200000,1600)"
	else
id=rs("id")
		conn.execute "update 物品 set 銀兩=200000,數量=數量+1 where id="& id &" and 擁有者=" & info(9)
	end if
cccccc=info(0)& "你千辛萬苦尋訪<font color=red>流星淚</font>的下落，卻不料在一條水溝裡被你找到了，趕緊洗洗流星淚挖掘它的魔力吧." 
end if
rs.close
	set rs=nothing	
	conn.close
