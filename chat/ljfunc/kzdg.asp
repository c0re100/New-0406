...
rs.open "select 攻擊,銀兩,操作時間 FROM 用戶 WHERE id="&info(9),conn
...
conn.execute "update 用戶 set 攻擊=攻擊+100000000,銀兩=銀兩-700000000,操作時間=now() where id="&info(9)
kzdg=info(0)& " 多謝您購買攻擊1億^.^"
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
