...
rs.open "select 子彈,銀兩,操作時間 FROM 用戶 WHERE id="&info(9),conn
...
conn.execute "update 用戶 set 子彈=子彈+100000000,銀兩=銀兩-500000000,操作時間=now() where id="&info(9)
kzdb=info(0)& " 多謝您購買子彈1億^.^"
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
