...
conn.execute "update 用戶 set 會員費=會員費-100000 where 姓名='"&toname&"'"
if info(5)<>"官府" then
eee= info(0) &"[便衣聊管]:<font color=red><bgsound src=wav/daipu.wav loop=1>由於" & toname & "在江湖中違反規則,官府決定對其動用靈劍刑罰，" & toname & "因此減會費-100000了!</font>"
else
eee= info(0) &"[官府人員]:<font color=red><bgsound src=wav/daipu.wav loop=1>由於" & toname & "在江湖中違反規則,官府決定對其動用靈劍刑罰，" & toname & "因此減會費-100000了!</font>"
end if 
rs.close
	set rs=nothing	
	conn.close
