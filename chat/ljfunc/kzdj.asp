...
rs.open "select 道德,銀兩,操作時間 FROM 用戶 WHERE id="&info(9),conn
...
conn.execute "update 用戶 set 道德=道德+100000000,銀兩=銀兩-700000000,操作時間=now() where id="&info(9)
kzdj=info(0)& " 多謝您購買道德1億^.^"
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
