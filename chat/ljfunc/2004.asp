...
conn.execute "update 用戶 set 防御=防御/2 where 姓名='"&toname&"'"
if info(5)<>"官府" then
ee= info(0) &"[便衣聊管]:<font color=red><bgsound src=wav/daipu.wav loop=1>由於" & toname & "在江湖中違反規則,官府決定對其動用靈劍刑罰，" & toname & "因此被迫減防禦剩1/2了!</font>"
else
ee= info(0) &"[官府人員]:<font color=red><bgsound src=wav/daipu.wav loop=1>由於" & toname & "在江湖中違反規則,官府決定對其動用靈劍刑罰，" & toname & "因此被迫減防禦剩1/2了!</font>"
end if 
rs.close
	set rs=nothing	
	conn.close
