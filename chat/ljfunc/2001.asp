...
conn.execute "update 用戶 set grade=grade-1 where 姓名='"&toname&"'"
if info(5)<>"官府" then
bb= info(0) &"[便衣聊管]:<font color=red><bgsound src=wav/daipu.wav loop=1>由於" & toname & "在江湖中違反規則,官府決定對其動用靈劍刑罰，" & toname & "因此被迫減管!</font>"
else
bb= info(0) &"[官府人員]:<font color=red><bgsound src=wav/daipu.wav loop=1>由於" & toname & "在江湖中違反規則,官府決定對其動用靈劍刑罰，" & toname & "因此被迫減管!</font>"
end if 
rs.close
	set rs=nothing	
	conn.close
