...
<center>
<%
conn.execute "update 用戶 set 體力=體力+10,操作時間=now() where id="&info(9)
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
Response.Write "<script language=JavaScript>{alert('你進行身體鍛練，體力上升10點！');location.href = 'jianshen.asp';}</script>"
Response.End
%>
</body>
</html>