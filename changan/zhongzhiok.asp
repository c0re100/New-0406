...
info=Session("info")
if Instr(LCase(Application("ljjh_useronlinename"))," "&LCase(info(0))&" ")=0 then
...
<center>
<%
conn.execute "update 用戶 set 魅力=魅力+5,操作時間=now() where id="&info(9)
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
Response.Write "<script language=JavaScript>{alert('你認認真真的種了花草，魅力上升5點！');location.href = 'zhongzhi.asp';}</script>"
Response.End
%>
</body>
</html>