...
<center>
<%
conn.execute "update 用戶 set 道德=道德+5,操作時間=now() where id="&info(9)
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
Response.Write "<script language=JavaScript>{alert('你愉快地喝完熱騰騰的咖啡後，道德上升5點！');location.href = 'hekafei.asp';}</script>"
Response.End
%>
</body>
</html>
