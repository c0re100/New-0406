...
rs.close
rs.open "select id from 絤珇 where 珇='ネ床' and 局Τ="&info(9),conn
If Rs.Bof OR Rs.Eof then
	conn.execute "insert into 絤珇(珇,局Τ,计秖,,糤计) values ('ネ床'," & info(9) &",1,'砰',1800)"
else
	id=rs("id")
	conn.execute "update 絤珇 set 糤计=1800,计秖=计秖+1 where id="&id
end if
rs.close
set rs=nothing
conn.close
