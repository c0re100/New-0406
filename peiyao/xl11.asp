...
rs.open "SELECT id FROM 珇 WHERE 局Τ=" & info(9) & " and 计秖>=5 and 珇='膓ホ'",conn
if rs.eof or rs.bof then 
	rs.close
	set rs=nothing
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('矗ボ膓ホぃ镑5ぃ巨');</script>"
	response.end
end if
kuangid=rs("id")
rs.close
...
laohuid=rs("id")
conn.Execute "update 珇 set 计秖=计秖-5 WHERE id="&kuangid
conn.Execute "update 珇 set 计秖=计秖-5 WHERE id="&iceid
conn.Execute "update 珇 set 计秖=计秖-1 WHERE id="&laohuid
rs.close
rs.open "select id from 絤珇 where 珇='旱義眏癌' and 局Τ=" & info(9),conn
If Rs.Bof OR Rs.Eof then
	conn.execute "insert into 絤珇(珇,局Τ,计秖,,糤计) values ('旱義眏癌'," & info(9) &",1,'ю阑',4000)"
else
	id=rs("id")
