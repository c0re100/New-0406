...
if rs.eof or rs.bof then 
	...
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('矗ボ膓ホぃ镑10ぃ巨');</script>"
	response.end
end if
kuangid=rs("id")
rs.close
...
liyuid=rs("id")
conn.Execute "update 珇 set 计秖=计秖-10 WHERE id="&kuangid
conn.Execute "update 珇 set 计秖=计秖-2 WHERE id="&liyuid
rs.close
rs.open "select id from 絤珇 where 珇='籘う' and 局Τ="&info(9)
If Rs.Bof OR Rs.Eof then
	conn.execute "insert into 絤珇(珇,局Τ,计秖,,糤计) values ('籘う',"& info(9) &",1,'緔',4000)"
else
	id=rs("id")
