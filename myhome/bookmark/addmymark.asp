...
%>
<%
rs.open "select user from bookmark where user='"&info(0)&"' and name='"&request.form("name")&"' and url='http://"&request.form("url")&"'",conn,1,1
if not rs.bof then
%>
<script language="Vbscript">
msgbox"您已經添加過此網站！",0,"提示"
history.back
</script>
<%
rs.close
...
rs.update "url",request.form("url")
