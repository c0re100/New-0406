...
<%else
rs.open"select milk from sheep where user='"&info(0)&"' and sheepname='"&sheepname&"'",conn,1,1
if rs.bof then
rs.close
conn.close
...
rs.open"select milkprice from rules",conn,1,1
...
<%
conn.execute"update 用戶 set 泡豆點數=泡豆點數+"&tempsplosh&" where 姓名='"&info(0)&"'"
	set rs=nothing	
	conn.close
	set conn=nothing
%>
...
<div align="center">
  ...
  cellpadding="0" height="20">
    <tr>
        <td class="p6"><font size="2">□-您當前的位置-&gt;江湖育兒院〉&gt;領工資&nbsp;&nbsp;&nbsp; 
  ...
  <table border="0" width="470" cellspacing="1" cellpadding="0" height="50">
    <tr>
      <td class="p2" width="100%">
        ...
        您已經成功拿到了你的孩子為你賺來的豆點：<%=tempsplosh%></font></p>
      </td>
    </tr>
  </table>
  </center>
