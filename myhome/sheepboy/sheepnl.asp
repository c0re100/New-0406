...
<!--#include file="data2.asp"-->
<%
rs.open"select feeddate,wuload,hungry,leinl,milk from sheep where sheepname='"&sheepname&"' and (user='"&info(0)&"' or sheep002='"&info(0)&"')",conn,1,1
...
tempdate=date
conn.execute"update sheep set leinl='"&leinl&"',feeddate='"&date&"',feedsheepday=feedsheepday+1,wuload='1' where sheepname='"&sheepname&"' and user='"&info(0)&"'"
...
Response.Write "<script language=JavaScript>{alert('靜坐完畢！');location.href = 'javascript:history.go(-1)';}</script>"
Response.End
rs.close
Set conn=Server.CreateObject("ADODB.CONNECTION")
Set rs=Server.CreateObject("ADODB.RecordSet")
...
rs.close
	...
	conn.close
	set conn=nothing
Response.Write "<script language=JavaScript>{alert('你們的小孩靜坐了8次！明天再來吧！');location.href = 'javascript:history.go(-1)';}</script>"
	Response.End
else
conn.execute"update sheep set leinl='"&leinl&"',wuload=wuload+1 where sheepname='"&sheepname&"' and (user='"&info(0)&"' or sheep002='"&info(0)&"')"
rs.close
conn.close
Set conn=Server.CreateObject("ADODB.CONNECTION")
Set rs=Server.CreateObject("ADODB.RecordSet")
