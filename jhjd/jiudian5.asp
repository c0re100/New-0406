...
<%
sql="select 配偶 from 用戶 where 姓名='" & my & "'  and 配偶<>'無' and 配偶<>'未定'"
...
rs.close
	...
	conn.close
	set conn=nothing
Response.Write "<script language=javascript>alert('你沒有配偶，不能訂購此酒宴');history.back();</script>"
response.end
else
peiou=trim(rs("配偶"))
...
set rs=connt.execute(sql)
if rs.eof or rs.bof then
sql="insert into 宴會列表(宴會名,擁有者,類型,內力,體力,金幣,數量,售價,參加者,時間) values ('"&wu&"','"&my&"','"&lx&"',"&nl&","&tl&","&jb&","&sl&","&yin&",'"&peiou&"',now())"
...
sd(197)="FFFDAF"
sd(198)="對"
sd(199)="<font color=FFFDAF>【好消息】"&my&"在江湖大酒店的"&wu&"廳為"&peiou&"舉行<font color=red>※"&lx&"※</font>宴會，"&peiou&"快去呀，呵呵..</font>"
sd(200)=session("nowinroom")
Application("ljjh_sd")=sd
Application.UnLock
end if			
Response.Write "<script language=javascript>alert('恭喜你，你的酒宴定桌成功，去告訴你的他(她)吧！');history.back();</script>"			
