...
if Application("ljjh_dalie")<>"老虎" then
	Response.Write "<script Language=Javascript>alert('提示：現在還沒有老虎，你要作什麼？！');window.close();</script>"
	response.end
end if
if info(2)>=7  then
	Response.Write "<script Language=Javascript>alert('提示：官府中人不可打獵！！');window.close();</script>"
	response.end
end if
...
if rs("內力")<300 or rs("體力")<300 or rs("銀兩")<10000 then
	rs.close
	set rs=nothing
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('提示：打獵需要內、體力各300，白銀10000兩！！');window.close();</script>"
	response.end
end if
session("dalie")=true
conn.execute "update 用戶 set 內力=內力-150,體力=體力-150,銀兩=銀兩-10000 where id="&info(9)
rs.close
set rs=nothing	
conn.close
set conn=nothing
