...
'校驗用戶
sql="SELECT * FROM 用戶 WHERE 姓名='" & name1 & "'"
Set Rs=conn.Execute(sql)
If not rs.bof and not rs.eof  Then  Response.Redirect "../error.asp?id=62"
sql="SELECT * FROM 用戶 WHERE 姓名='" & name & "' and 密碼='" & pass & "'"
Set Rs=conn.Execute(sql)
...
Set conn=Server.CreateObject("ADODB.CONNECTION")
Set rs=Server.CreateObject("ADODB.RecordSet")
bbs="DBQ="+server.mappath("../bbs/ljwjadsd.mdb")+";DefaultDir='';DRIVER={Microsoft Access Driver (*.mdb)};"
conn.open bbs
'校驗用戶
