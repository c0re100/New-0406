...
dim rs
dim filename
sql="select 姓名,性別,門派,身份,allvalue,mvalue,times,門派基金 from 用戶 where 門派='" & info(5) &"'order by 門派基金 desc"
Set rs= Server.CreateObject("ADODB.Recordset")
...
bordercolordark="#FFFFFF" cellpadding="0" bordercolor="#000000">
    <tr bgcolor="#990000"> 
      ...
      <td align="center" height="16" width="13%"><font color="#FFFFFF">門派基金</font></td>
    </tr>
    <%do while not rs.eof%>
    <tr> 
      ...
      <td align="center" width="13%"><font color="#FFDDCC"><%=rs("門派基金")%> 
        </font></td>
    </tr>
    <%
rs.movenext
...
<tr>
      <td width="100%" bgcolor="#0033CC"><font size="2" color="#ffffff">弟子向門派貢獻基金後10位排行</font></td>
</tr>
</table>
<%
filename=0
sql="select * from 用戶 where 門派='" & info(5) &"'order by -門派基金 desc"
Set rs= Server.CreateObject("ADODB.Recordset")
rs.open sql,conn,1,1
if rs.eof and rs.bof then
