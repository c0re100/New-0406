...
function doPreload(){
	var the_images = new Array('image/b.gif','image/b.gif','image/c.gif','image/d.gif','image/e.gif','image/f.gif','image/g.gif');
...
function checkState(f){ 
	...
	if(flag==1) alert("現在正在筏木！"); 
	if(flag==2) alert("現在正在蓋房！"); 
	 
} 
function chechstone(f){
       if(f<100) alert("對不起，你的木材不夠蓋房！");
}
...
function gocai(){ 
	...
	hiddenstone();
        if(stonenum>900) alert("請注意木材數量不能大於999！"); 
	flag=1;
        changeImage(caikuang,"image/d.gif");
	caikuang.style.visibility="visible";
	sAniMove(caikuang,4,200,416,25,"goOver()"); 
} 
...
//	alert (steelstate);
        changeImage(liantie,"image/d.gif");	
        liantie.style.visibility="visible";
	sAniMove(liantie,7,200,395,178,"goOver1()"); 
} 
function goOver1(){ 
	changeImage(liantie,"image/b.gif"); 
        steelstate=steelstate+1;
//	alert(steelstate);
